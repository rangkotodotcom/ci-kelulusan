...
class Importnilaius extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->session->userdata('email') == '') {
            ...
            $this->load->view('master/import/importnilaius', $data);
    ...
    {
        force_download('./assets/template/template_import_nilai_usbn.xlsx', NULL);
    }
    ...
    public function upload()
    {
        ...
        if (!$this->upload->do_upload()) {
            ...
            //upload gagal
            $this->session->set_flashdata('alert', 'Import Nilai USBN Siswa Gagal');
            //redirect halaman
            redirect(base_url('master/importnilaius/'));
        } else {
            ...
            if ($judul == 'Blangko Import Nilai USBN') {
                ...
                foreach ($sheet as $row) {
                    ...
                    $cek = $this->db->get_where('t_n_us', ["no_pes" => $no_pes])->num_rows();
                ...
                $history = [
                    'kegiatan' => 'Import Nilai USBN Siswa',
                    'oleh' => $this->session->userdata('email'),
                    'waktu' => NULL
                ];

                if ($cek > 0) {
                    $this->db->update_batch('t_n_us', $us, 'no_pes');
                } else {
                    $this->db->insert_batch('t_n_us', $us);
                }
                ...
                //upload success
                $this->session->set_flashdata('alert', 'Import Nilai USBN Siswa Berhasil');
                //redirect halaman
